...
import adventure.input.mouse.listener.DefaultMouseListener;
...
public class GamePanel extends JPanel {
    ...
    public void paintComponent(Graphics graphics) {
        ...
        // if (!AdventureGame.getInstance().getTickManager().isInitialized()) return;

        if (AdventureGame.getInstance().getActiveState() == null) return;

        // AdventureGame.getInstance().getTickManager().render(graphics);
        AdventureGame.getInstance().getActiveState().render(graphics);
    }
